    ...
    return (
        <div className="ds-color-manager">
            ...
            <div className="ds-add-color">
                <h4>{__('Add New Color', 'ds-studio')}</h4>
                
                <TextControl
                    label={__('Color Name', 'ds-studio')}
                    value={newColorName}
                ...
                <div className="ds-color-picker-wrapper">
                    <label>{__('Color Value', 'ds-studio')}</label>
                    <ColorPicker
                        color={newColorValue}
                ...
                >
                    {__('Add Color', 'ds-studio')}
                </Button>
            </div>

            {/* Color Palette */}
            <div className="ds-color-palette">
                <h4>{__('Color Palette', 'ds-studio')}</h4>
                
                {colors.length === 0 && (
                    <Notice status="info" isDismissible={false}>
                        {__('No colors in palette. Add your first color above.', 'ds-studio')}
                    </Notice>
                )}
            ...
            <div className="ds-save-section">
                <Button
                    ...
                    isBusy={isLoading}
                >
                    {isLoading ? __('Saving...', 'ds-studio') : __('Save to theme.json', 'ds-studio')}
                </Button>
            </div>
        </div>
    );
