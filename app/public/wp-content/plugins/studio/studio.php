...
/**
 * Plugin Name: Studio
 * Plugin URI: https://github.com/yourusername/studio
 * Description: AI-powered WordPress block system with design tokens and visual component builder
 * Version: 1.0.0
...
define('STUDIO_PLUGIN_PATH', plugin_dir_path(__FILE__));
...
class Studio {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_studio_save_theme_json', array($this, 'save_theme_json'));
        add_action('wp_ajax_studio_get_theme_json', array($this, 'get_theme_json'));
        
        // Add manual sync handler
        add_action('wp_ajax_studio_manual_sync_to_theme_json', array($this, 'manual_sync_to_theme_json_ajax'));
        
        // Add import from theme.json handler
        add_action('wp_ajax_studio_import_from_theme_json', array($this, 'import_from_theme_json_ajax'));
        
        // Add component management AJAX handlers
        add_action('wp_ajax_studio_save_component', array($this, 'save_component_ajax'));
        add_action('wp_ajax_studio_delete_component', array($this, 'delete_component_ajax'));
    }
    ...
    public function init() {
        ...
        load_plugin_textdomain('studio', false, dirname(plugin_basename(__FILE__)) . '/languages');
    }
    ...
    public function enqueue_block_editor_assets() {
        ...
        wp_enqueue_script(
            'the-studio',
            STUDIO_PLUGIN_URL . 'assets/js/studio.js',
        ...
        wp_enqueue_style(
            'the-studio-styles',
            STUDIO_PLUGIN_URL . 'assets/css/studio.css',
            array(),
            '2.0.0'
        );
        ...
        wp_localize_script('the-studio', 'studioBlockStyles', array(
            'nonce' => wp_create_nonce('studio_block_styles_nonce'),
            'ajaxUrl' => admin_url('admin-ajax.php')
        ));
    }
    ...
    public function save_theme_json() {
        // Verify nonce
        if (!wp_verify_nonce($_POST['nonce'], 'studio_nonce')) {
    ...
    public function manual_sync_to_theme_json_ajax() {
        // Add error logging
        error_log('Studio: Manual sync started');
        
        // Verify nonce
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'studio_nonce')) {
            error_log('Studio: Nonce verification failed');
            wp_send_json_error('Security check failed');
        }
        
        // Check user permissions
        if (!current_user_can('edit_theme_options')) {
            error_log('Studio: Permission check failed');
            wp_send_json_error('Insufficient permissions');
        }
        
        // Load Studio tokens
        $studio_json_path = plugin_dir_path(__FILE__) . 'studio.json';
        error_log('Studio: Looking for studio.json at: ' . $studio_json_path);
        
        if (!file_exists($studio_json_path)) {
            error_log('Studio: studio.json not found');
        ...
        if (!$studio_data || !isset($studio_data['colors'])) {
            error_log('Studio: No colors found in studio.json');
            wp_send_json_error('No colors found in studio.json');
        }
        
        error_log('Studio: Found ' . count($studio_data['colors']) . ' colors');
        
        // Load existing theme.json
        $theme_json_path = get_stylesheet_directory() . '/theme.json';
        error_log('Studio: Theme.json path: ' . $theme_json_path);
        ...
        if (file_exists($theme_json_path)) {
            ...
            error_log('Studio: Loaded existing theme.json');
        }
        ...
        error_log('Studio: Converted ' . count($our_colors) . ' colors for sync');
        ...
        error_log('Studio: File write result: ' . ($result !== false ? 'SUCCESS' : 'FAILED'));
    ...
    public function import_from_theme_json_ajax() {
        // Add error logging
        error_log('Studio: Import from theme.json started');
        ...
        if (!file_exists($theme_json_path)) {
            error_log('Studio: theme.json not found');
        ...
        if (!$theme_data || !isset($theme_data['settings']['color']['palette'])) {
            error_log('Studio: No colors found in theme.json');
            wp_send_json_error('No colors found in theme.json');
        }
        
        error_log('Studio: Found ' . count($theme_data['settings']['color']['palette']) . ' colors');
        ...
        if (file_exists($studio_json_path)) {
            $studio_content = file_get_contents($studio_json_path);
            $studio_data = json_decode($studio_content, true) ?: [];
            error_log('Studio: Loaded existing studio.json');
        }
        ...
        error_log('Studio: Converted ' . count($our_colors) . ' colors for import');
...
// new Studio_Design_Token_Manager();
