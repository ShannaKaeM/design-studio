...
class Studio_Design_Token_Manager {
    ...
    public function __construct() {
        ...
        // Add AJAX handlers
        add_action('wp_ajax_studio_get_design_tokens', array($this, 'get_design_tokens_ajax'));
        add_action('wp_ajax_studio_save_design_tokens', array($this, 'save_design_tokens_ajax'));
        add_action('wp_ajax_studio_manual_sync_to_theme_json', array($this, 'manual_sync_to_theme_json_ajax'));
        add_action('wp_ajax_studio_clear_studio_cache', array($this, 'clear_studio_cache_ajax'));
        add_action('wp_ajax_studio_save_category', array($this, 'save_category_ajax'));
        add_action('wp_ajax_studio_delete_category', array($this, 'delete_category_ajax'));
    }
    ...
    /**
     * Get design tokens formatted for JavaScript
     * 
     * @return array Tokens formatted for JavaScript consumption
     */
    public function get_tokens_for_js() {
        ...
        if (isset(($tokens ?: $this->tokens_data)['typography']['fontSizes'])) {
            foreach (($tokens ?: $this->tokens_data)['typography']['fontSizes'] as $size_name => $size_value) {
                ...
                if (is_array($size_value)) {
                    $size = isset($size_value['value']) && is_string($size_value['value']) ? $size_value['value'] : '1rem';
                    $theme_json['settings']['typography']['fontSizes'][] = [
                        'name' => $size_value['name'] ?? ucfirst($size_name),
                        'slug' => $size_name,
                        'size' => $size
                    ];
                } else {
                    // Ensure it's a string value
                    $size = is_string($size_value) ? $size_value : '1rem';
                    $theme_json['settings']['typography']['fontSizes'][] = [
    ...
    public function manual_sync_to_theme_json() {
        ...
        $existing_theme_json['settings']['color'] = array_merge(
            ...
            [
                ...
                'defaultPalette' => false, // Hide default WordPress colors to show only Studio colors
                'palette' => $our_colors,
                'gradients' => $our_gradients
            ]
        );
        
        // Enhanced typography settings for better WordPress control population
        if (!isset($existing_theme_json['settings']['typography'])) {
            $existing_theme_json['settings']['typography'] = [];
        }
        
        $existing_theme_json['settings']['typography'] = array_merge(
            $existing_theme_json['settings']['typography'],
            [
                'customFontSize' => true,
                'fontSizes' => $our_typography,
                'dropCap' => false,
                'fontStyle' => true,
                'fontWeight' => true,
                'letterSpacing' => true,
        ...
        // Enhanced spacing settings for WordPress controls
        if (!isset($existing_theme_json['settings']['spacing'])) {
            $existing_theme_json['settings']['spacing'] = [];
        }
        
        $existing_theme_json['settings']['spacing'] = array_merge(
            $existing_theme_json['settings']['spacing'],
            [
                'customSpacingSize' => true,
                'spacingSizes' => $our_spacing,
    ...
    public function clear_studio_cache_ajax() {
        if (!check_ajax_referer('studio_nonce', 'nonce', false)) {
